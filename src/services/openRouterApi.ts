...
// Available models for analysis testing
// Note: These model names should be verified against current OpenRouter availability
export const ANALYSIS_MODELS = {
  'mistral/mistral-medium-latest': 'Mistral Medium (Baseline)',
  'google/gemma-2-7b-it': 'Gemma 2 7B Instruct',
...
export class OpenRouterApiService {
  ...
  async processDocumentWithOCR(file: File): Promise<ProcessedDocument> {
    try {
      console.log('Document processing with Open Router OCR for:', file.name);
      ...
      if (response.choices?.[0]?.message?.content) {
        ocrContent = response.choices[0].message.content;
      }
      ...
      return {
        ...
        pageCount: 1 // We'll estimate this from content or extract from response
      };
    } catch (error) {
      console.error('Open Router OCR processing failed:', error);
  ...
  async analyzeDocumentForCompliance(
    ...
    model: AnalysisModel = 'mistral/mistral-medium-latest'
  ): Promise<string> {
    try {
      console.log(`Starting FEMA compliance analysis with model: ${model}`);

      // APR-Guardian comprehensive review prompt (same as Mistral version)
      const reviewPrompt = `
      **ROLE & MISSION:**
